...
function printBanner() {
 ...
 | |  | || | / ___ \\
 |_|  |_|___/_/   \\_\\\x1b[0m
\x1b[2mManager in Action — discovery, spec, verification\x1b[0m
`);
}
...
// their content passed through as-is (no placeholders to resolve — mia has
// none), other files are copied verbatim.
...
function installToTarget(targetKey, cwd) {
	...
	const extras = fs.readdirSync(SKILL_SRC).filter((f) => f !== 'SKILL.md');
	if (extras.length > 0) {
...
async function runInteractiveInstall() {
	...
	try {
		...
		console.error(
			'❌ Interactive mode requires the "prompts" package, which should have been installed automatically.\n' +
				'   Try: npx @nullbotdotexe/mia@latest install',
		);
		process.exit(1);
	}
	...
	const { targets } = await prompts(
		{
			type: 'multiselect',
			name: 'targets',
			message: 'Select the target IDE(s) / tool(s) for mia',
			instructions: false,
	...
	for (const targetKey of targets) {
		...
		console.log(`✅ mia installed to ${TARGETS[targetKey].label}`);
	}
	console.log(
		'\n🎉 Done! Start with "/mia grill" to open a discovery interview for a new request/feature.',
	);
}
...
async function main() {
	if (command === 'install') {
		if (args[1] === '--all') {
			...
			console.log(`✅ mia installed to ${TARGETS['claude-project'].label}`);
			console.log(
				'\n🎉 Done! Start with "/mia grill" to open a discovery interview for a new request/feature.',
			);
		} else {
		...
		console.log(`
Usage:
  mia install              Interactive: pick the target IDE(s)/tool(s)
  mia install --all        Install to Claude Code (project), no prompts

This installs:
  .claude/skills/mia/      Discovery interview (grill) + verification (verify) workflow
  .claude/skills/mia/references/spec-template.md   Authoritative spec.md structure

Mia operates per-request/feature inside a .spec/<RequestName>/ folder in the project it's
installed into — not inside this package. Commands: /mia grill, /mia status, /mia approve,
/mia amend, /mia verify.
`);
	}
}
